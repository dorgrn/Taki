        ...
        createDeck: function () {
            ...
            cardFactory.values.forEach(function (value) {
                for (var i = 0; i < manager.CARD_MULTIPLES; i++) {
                    cardFactory.colors.forEach(
                        function (color) {
                            manager.deck.push(cardFactory.createCard(color, "number", getFilename(value, color), value));
                        }
                    );
                }
            });
            ...
            cardFactory.superCards.forEach(function (type) {
                for (var i = 0; i < manager.SUPER_CARDS; i++) {
                    manager.deck.push(cardFactory.createCard("colorful", type, getFilename(type)));
                }
            });
