const UI = ( function(){
    ...
    function dealCardsToPlayerHand(player, cardsAmount){
        for (var i = player.hand.cards.length - 1; i > player.hand.cards.length - 1 - cardsAmount; i--){
            createPlayerCard(player, player.hand.cards[i]);
        }
    }

    function createPlayzoneCard(card){
    ...
    function createPlayerCard(player, card){
        var cardImage = document.createElement("IMG");
    ...
    function setHoverOnUserCards() {
        ...
        for (var i = 0; i < cards.length; i++) {
            ...
            cards[i].addEventListener("click", function(){
                UI.onCardClick.notify({cardName: this.alt});
    ...
    return {
        ...
        init : function(){
            ...
            createPlayzoneCard(manager.getPlayZoneTop());
