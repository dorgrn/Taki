var gameController = (function (logic, view) {
    ...
    function gameEnded(quit) {
        // TODO: generify in advanced
        var winnerPlayer = (quit ? manager.players[manager.getNextPlayer()] : manager.getActivePlayer());
        stats.gameWatch.stop();
        clearInterval(view.elapsedTimeInterval);
        view.showEndGameMenu(winnerPlayer);
    }
    ...
    return {
        init: function () {
            ...
            view.onPlayerQuit.attach(function (args) {
                // TODO: in advanced add a boolean whether a player has quit. check with manager
                gameEnded(true);
            });
